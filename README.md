...
- `linebreak` : 文献を指定の文字数で改行します．複数の数字を入力すると，その数だけ改行します．1つのときにもArray型として渡す必要があり，例えば10文字目で改行する際には`(10,)`と入力します．（例：`linebreak: ("Ahrendt, W. R. ".len(),)`）
...
> 文字数を指定する際には，日本語文献に注意してください．Typstでは，全角文字1つが3文字としてカウントされます．
